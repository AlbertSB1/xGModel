...
def estimateXG(collection, query, fields_to_encode):
    ...
    try:
        df = df[['dist', 'bodyPart', 'x', 'y', 'prevEventType', 'penalty', 'angle', 'bigChance', 'isGoal', 'match']]
    except:
        df = df[['dist', 'bodyPart', 'x', 'prevEventType', 'penalty', 'angle', 'bigChance', 'isGoal', 'match']]
    df['xG'] = 0
...
#Set parameters to compare WSl to League Two
query = {'league':{'$nin':['WSL', 'League Two']}} #exclude WSL and League Two from train data
...
query = {'league': 'WSL'}
wsl_df1 = estimateXG(collection, query, fields_to_encode)
wsl_df1.to_csv('wsl_df2.csv')

#estimate number of goals from League One
query = {'league': 'League Two'}
lg2_df = estimateXG(collection, query, fields_to_encode)
lg2_df.to_csv('lg2_df.csv')

#Set parameters to compare WSl to League One
query = {'league':{'$nin':['WSL', 'League One']}} #exclude WSL and League Two from train data
fields_to_train = ['x', 'dist', 'angle', 'prevEventType', 'bodyPart']
fields_to_encode = ['prevEventType', 'bodyPart']
...
#estimate number of goals from WSL
query = {'league': 'WSL'}
wsl_df2 = estimateXG(collection, query, fields_to_encode)
wsl_df2.to_csv('wsl_df1.csv')

#estimate number of goals from League One
query = {'league': 'League One'}
lg1_df = estimateXG(collection, query, fields_to_encode)
lg1_df.to_csv('lg1_df.csv')

#Set parameters to compare WSl to English Men's Game
query = {'league':{'$nin':['WSL', 'Championship', 'League One', 'League Two', 'League Cup',
                           'Premier League']}} #exclude WSL and English men's game from train data
fields_to_train = ['x', 'dist', 'angle', 'prevEventType', 'bodyPart']
fields_to_encode = ['prevEventType', 'bodyPart']
...
#estimate number of goals from WSL
query = {'league': 'WSL'}
wsl_df3 = estimateXG(collection, query, fields_to_encode)
wsl_df3.to_csv('wsl_df_all.csv')

#estimate number of goals from English men's game
query = {'league': {'$in':['Championship', 'League One', 'League Two', 'League Cup',
                           'Premier League']}}
all_df = estimateXG(collection, query, fields_to_encode)
